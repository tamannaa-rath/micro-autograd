...
How it works
The main idea is that every time you do something like a * b + c, the engine builds a graph. When you call .backward(), it walks through that graph in reverse order and calculates how much every single number affected the final result.
...
Bash
python train.py
It should print out the loss decreasing as the network gets better at predicting the targets.
